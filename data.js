window.PORTFOLIO_DATA = {
  ...
  projects: [
    ...
    },
    {
      ...
      details: "- 로컬 2인용 키보드 대전 및 타격 시스템:\n  - 한 대의 PC(단일 키보드)로 2명이 동시에 대전할 수 있는 독립적 조작 체계(P1: WASD / P2: 방향키) 설계\n  - 잽(Light) 공격, 강(Heavy) 공격 및 넉백 반동(Recoil) 메커니즘을 통한 밀고 밀리는 타격감 구현\n- 매 라운드 무작위로 변경되는 3가지 모드 맵 기믹:\n  - 기본 원형 전장인 'Large Circle', 시간에 따라 안전지대가 좁아지는 'Shrinking Circle'(자기장 기믹), 주기적인 경고 표시 후 동적 장애물이 소환되는 'Obstacle Heavy' 기믹 구현\n- 실시간 아이템 파밍 및 버프 시스템:\n  - 맵 내부 무작위 가중치 기반으로 4종(넉백 강화, 방패(넉백 감소), 폭탄 장전, 이동속도 대시)의 아이템 스폰 연동\n- 게임플레이 코어 및 물리 제어 로직 구현:\n  - 3판 2선승제 매치 및 라운드 루프(준비 -> 진행 -> 종료 및 판정) 제어\n  - 캐릭터 가감속 관성 처리 및 넉백 상태(Knockback State) 시 조작 제한 등 물리 기반 상태 머신 제어\n  - 타격 시 공격자에게도 작용하는 반동(Self-Push) 물리력 연동 및 오디오 피드백 강화\n- 맵 이탈(Ring Out) 및 스마트 스폰 시스템:\n  - 맵 중심(Vector3.zero)과 플레이어 간 거리를 실시간으로 체크하여 외곽 이탈 시 즉시 패배를 판정하는 탈락 로직 개발\n  - 리플렉션(Reflection) 기술을 적용해 동적으로 축소되는 맵의 실시간 반지름을 추적, 안전 구역 내부에만 아이템을 드롭하는 스마트 아이템 스포너(ItemSpawner2D) 설계",
