  ...
  projects: [
    {
      ...
      details: "- 게임플레이 코어 로직 및 UI/UX 설계:\n  - 9가지 등급의 광석 채굴, 인벤토리 시스템 및 실시간 자원 반영 상점 기능 개발\n  - 드릴 속도, 파워, 범위 등 다각화된 스탯 강화를 위한 모듈형 업그레이드 아키텍처 수립\n- 콘텐츠 기믹 및 보상 파이프라인 구현:\n  - 탐험 시 무작위로 만나는 미니게임 이벤트 맵 로직 설계 및 성공/실패 보상 지급 판정 루프 완성\n  - 사용자 데이터 영속성을 위한 로컬/스팀 클라우드 데이터 세이브/로드 연동\n- QA 및 퍼블리싱:\n  - 데모 버전 스팀(Steam) 런칭을 위한 연동 작업 및 클라이언트 빌드 최적화(FPS 확보) 및 QA 수행",
